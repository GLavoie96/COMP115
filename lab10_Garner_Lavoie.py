...
practice_if(80, 80)?
"""
#Your answer is: It will print out "Nicely done!" since 80 isnt not greater than 80
...
Your answer is: They are the same but differ in ways once they find the target, index_1 uses a simple loop to find the target
while index_2 uses an index variable to store the value and then "break" out of the loop
...
functionality? Why? (5 marks)
Your answer is: They are different because index_2 is looking for the first occurence
to break out of the loop while index_3 will update the index with each occurance and then 
return the final occurance of the target.
...
def more_than_2(words):
    ...
    result = [string for string, count in counts.items() if count > 2]
    return result
